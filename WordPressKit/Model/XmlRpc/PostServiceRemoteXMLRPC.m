...
{
    NSArray *parameters = [blog getXMLRPCArgsWithExtra:postID withDBRecordId:blog.id];
           ...
           failure:(void (^)(NSError *))failure
{
    ...
    [self.api callMethod:@"metaWeblog.newPost"
              parameters:parameters
                 success:^(AFHTTPRequestOperation *operation, id responseObject) {
                         post.postID = [NSNumber numberWithUnsignedLongLong:[responseObject longLongValue]];
           ...
           failure:(void (^)(NSError *))failure
{
    ...
    blog.password = [self.dataModel readKeyChainWithId:blog.id];
